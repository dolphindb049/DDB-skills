...
# 设置日志配置
logger_stock_cashflow = logging.getLogger('logger_stock_cashflow')
logger_stock_cashflow.setLevel(logging.INFO)
# 日志输出到文件
fileHandler_stock_cashflow = logging.FileHandler(basic.logDir + "/stock_cashflow.log")
formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
fileHandler_stock_cashflow.setFormatter(formatter)
# 日志输出到控制台
console_stock_cashflow = logging.StreamHandler() 
console_stock_cashflow.setFormatter(formatter)
# 将 Handler 添加到 Logger
logger_stock_cashflow.addHandler(fileHandler_stock_cashflow)
logger_stock_cashflow.addHandler(console_stock_cashflow)
    ...
    while retry_count < max_retries:
        try:
            ...
            if data is not None:
                ...
                logger_stock_cashflow.info("Data of %s %s has been written.", dataSource, ts_code)
                return
            else:
                s.run('writeLogLevel(WARNING, "'+'easyTushareImport - '+dataSource+' - '+dataSource+' '+ts_code+' has no data.")')
                logger_stock_cashflow.warning("%s %s has no data.", dataSource, ts_code)
                return
            ...
            logger_stock_cashflow.warning("Failed to get data of %s %s. Will retry in 10 seconds. Error: %s", dataSource, ts_code, e)
            time.sleep(10)
    # 如果达到最大重试次数，记录错误并抛出异常
    logger_stock_cashflow.error("Failed to get data of %s after %d attempts. Exiting.", dataSource, max_retries)
...
def main(session, startDate, endDate, token, dataSource, maxRetries):
    ...
    # python log 日志
    logger_stock_cashflow.info("The %s data import is complete.", dataSource)
