...
# 设置日志配置
logger_stock_monthly = logging.getLogger('logger_stock_monthly')
logger_stock_monthly.setLevel(logging.INFO)
# 日志输出到文件
fileHandler_stock_monthly = logging.FileHandler(basic.logDir + "/stock_monthly.log")
formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
fileHandler_stock_monthly.setFormatter(formatter)
# 日志输出到控制台
console_stock_monthly = logging.StreamHandler() 
console_stock_monthly.setFormatter(formatter)
# 将 Handler 添加到 Logger
logger_stock_monthly.addHandler(fileHandler_stock_monthly)
logger_stock_monthly.addHandler(console_stock_monthly)
...
def get_data(trade_date, appender, token, dataSource, s, maxRetries):
    ...
    while retry_count < max_retries:
        try:
            data = pro.monthly(trade_date=trade_date)
            data['trade_date'] = pd.to_datetime(data['trade_date'], format='%Y%m%d')
            data['update_time'] = datetime.now()
            appender.append(data)
            logger_stock_monthly.info("Data of %s %s has been written.", dataSource, trade_date)
            return
            ...
            logger_stock_monthly.warning("Failed to get data of %s %s. Will retry in 10 seconds. Error: %s", dataSource, trade_date, e)
            time.sleep(10)
    # 如果达到最大重试次数，记录错误并抛出异常
    logger_stock_monthly.error("Failed to get data of %s after %d attempts. Exiting.", dataSource, max_retries)
...
def main(session, startDate, endDate, token, dataSource, maxRetries):
    ...
    logger_stock_monthly.info("The %s data import is complete.", dataSource)