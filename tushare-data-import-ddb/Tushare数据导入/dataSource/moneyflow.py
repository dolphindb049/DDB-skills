...
# 设置日志配置
logger_moneyflow = logging.getLogger('logger_moneyflow')
logger_moneyflow.setLevel(logging.INFO)
# 日志输出到文件
fileHandler_moneyflow = logging.FileHandler(basic.logDir + "/moneyflow.log")
formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
fileHandler_moneyflow.setFormatter(formatter)
# 日志输出到控制台
console_moneyflow = logging.StreamHandler() 
console_moneyflow.setFormatter(formatter)
# 将 Handler 添加到 Logger
logger_moneyflow.addHandler(fileHandler_moneyflow)
logger_moneyflow.addHandler(console_moneyflow)
...
def get_data(trade_date, appender, token, dataSource, s, maxRetries):
    ...
    while retry_count < max_retries:
        try:
            ...
            logger_moneyflow.info("Data of %s %s has been written.", dataSource, trade_date)
            return
            ...
            logger_moneyflow.warning("Failed to get data of %s %s. Will retry in 10 seconds. Error: %s", dataSource, trade_date, e)
            time.sleep(10)
    # 如果达到最大重试次数，记录错误并抛出异常
    logger_moneyflow.error("Failed to get data of %s after %d attempts. Exiting.", dataSource, max_retries)
...
def main(session, startDate, endDate, token, dataSource, maxRetries):
    ...
    # python log 日志
    logger_moneyflow.info("The %s data import is complete.", dataSource)
