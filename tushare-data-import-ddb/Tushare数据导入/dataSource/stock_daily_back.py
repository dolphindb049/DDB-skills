...
# 设置日志配置
logger_stock_daily_back = logging.getLogger('logger_stock_daily_back')
logger_stock_daily_back.setLevel(logging.INFO)
# 日志输出到文件
fileHandler_stock_daily_back = logging.FileHandler(basic.logDir + "/stock_daily_back.log")
formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
fileHandler_stock_daily_back.setFormatter(formatter)
# 日志输出到控制台
console_stock_daily_back = logging.StreamHandler() 
console_stock_daily_back.setFormatter(formatter)
# 将 Handler 添加到 Logger
logger_stock_daily_back.addHandler(fileHandler_stock_daily_back)
logger_stock_daily_back.addHandler(console_stock_daily_back)
...
def get_data(ts_code, appender, startDate, endDate, token, dataSource, s, maxRetries):
    ...
    while retry_count < max_retries:
        try:
            data = ts.pro_bar(ts_code=ts_code, adj='hfq', start_date=startDate, end_date=endDate, ma=[5, 20, 50], factors=['tor', 'vr'], adjfactor=True)
            if data is not None:
                ...
                logger_stock_daily_back.info("Data of %s %s has been written.", dataSource, ts_code)
                return
            else:
                s.run('writeLogLevel(WARNING, "'+'easyTushareImport - '+dataSource+' - '+dataSource+' '+ts_code+' has no data.")')
                logger_stock_daily_back.warning("%s %s has no data.", dataSource, ts_code)
                return
            ...
            logger_stock_daily_back.warning("Failed to get data of %s %s. Will retry in 10 seconds. Error: %s", dataSource, ts_code, e)
            time.sleep(10)
    # 如果达到最大重试次数，记录错误并抛出异常
    logger_stock_daily_back.error("Failed to get data of %s after %d attempts. Exiting.", dataSource, max_retries)
...
def main(session, startDate, endDate, token, dataSource, maxRetries):
    ...
    # 初始化库表：沪深股票 - 行情数据 - 日线复权行情（后复权）
    scripts = """
    use DolphinDBModules::EasyTushare::createDBTB
    createStockDailyBack()
    """
    #s.run(scripts)
    ...
    logger_stock_daily_back.info("The %s data import is complete.", dataSource)