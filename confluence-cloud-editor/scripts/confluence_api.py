...
def markdown_to_storage_html(md_text: str, md_base_dir: Path, image_width: int = 900):
    ...
    while i < len(lines):
        ...
        if i + 1 < len(lines) and is_table_line(line) and is_table_sep(lines[i + 1]):
            ...
            html_parts.append(
                "<tr>" + "".join(f"<th><p>{format_inline_markdown(c)}</p></th>" for c in header_cells) + "</tr>"
            )
            i += 2
            while i < len(lines) and is_table_line(lines[i]):
                ...
                if is_table_sep(lines[i]):
                    i += 1
                    continue
                # Keep column count consistent with header count.
                ...
                html_parts.append(
                    "<tr>" + "".join(f"<td><p>{format_inline_markdown(c)}</p></td>" for c in row_cells) + "</tr>"
                )
                i += 1
